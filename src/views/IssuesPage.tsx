...
export default function IssuesPage() {
  ...
  const workflowMutation = useMutation({
    ...
    onError: (error) => {
      setSaveNote(null);
      setFormError(readableApiError(error, locale, locale === "ru" ? "?? ??????? ????????? workflow." : locale === "he" ? "?? ???? ????? ?? ?-workflow." : "Failed to save workflow"));
    },
  });
  ...
  const openMediaMutation = useMutation({
    ...
    onError: (error) => {
      setSaveNote(null);
      setFormError(readableApiError(error, locale, locale === "ru" ? "?? ??????? ??????? ???????? ????????." : locale === "he" ? "?? ???? ????? ?? ???? ????? ?? ?????." : "Failed to open issue media"));
    },
  });
  ...
  const bulkWorkflowMutation = useMutation({
    ...
    onError: (error) => {
      setSaveNote(null);
      setFormError(readableApiError(error, locale, locale === "ru" ? "?? ??????? ????????? ???????? ??????????." : locale === "he" ? "?? ???? ????? ????? ?????." : "Failed to apply bulk update"));
    },
  });
